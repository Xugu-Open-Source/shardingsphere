...
package org.apache.shardingsphere.db.protocol.xugu.codec;
 ...
 */
public final class XuguPacketCodecEngine implements DatabasePacketCodecEngine {
    ...
    public void encode(final ChannelHandlerContext context, final DatabasePacket message, final ByteBuf out) {
        XuguPacketPayload payload = new XuguPacketPayload(prepareMessageHeader(out).markWriterIndex(), context.channel().attr(CommonConstants.CHARSET_ATTRIBUTE_KEY).get());
        try {
            ...
            new XuguErrPacket(new UnknownSQLException(ex).toSQLException()).write(payload);
        } finally {
            if (out.readableBytes() - PAYLOAD_LENGTH - SEQUENCE_LENGTH < MAX_PACKET_LENGTH) {
                updateMessageHeader(out, context.channel().attr(XuguConstants.SEQUENCE_ID_ATTRIBUTE_KEY).get().getAndIncrement());
            } else {
    ...
    private void writeMultiPackets(final ChannelHandlerContext context, final ByteBuf byteBuf) {
        ...
        AtomicInteger sequenceId = context.channel().attr(XuguConstants.SEQUENCE_ID_ATTRIBUTE_KEY).get();
    ...
    @Override
    public XuguPacketPayload createPacketPayload(final ByteBuf message, final Charset charset) {
        return new XuguPacketPayload(message, charset);
    }
}
