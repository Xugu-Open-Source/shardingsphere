 ...
 */
public final class XuguTimestampBinlogProtocolValue implements XuguBinlogProtocolValue {
    
    @Override
    public Serializable read(final XuguBinlogColumnDef columnDef, final XuguPacketPayload payload) {
        int seconds = payload.readInt4();
        return 0 == seconds ? XuguTimeValueUtils.DATETIME_OF_ZERO : DateTimeFormatterFactory.getStandardFormatter().format(new Timestamp(seconds * 1000L).toLocalDateTime());
    }
}
