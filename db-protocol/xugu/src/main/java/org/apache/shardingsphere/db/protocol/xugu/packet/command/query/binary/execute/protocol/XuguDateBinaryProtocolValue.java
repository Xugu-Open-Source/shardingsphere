 ...
 */
public final class XuguDateBinaryProtocolValue implements XuguBinaryProtocolValue {
    ...
    private Timestamp getTimestampForDate(final XuguPacketPayload payload) {
        return Timestamp.valueOf(LocalDate.of(payload.readInt2(), payload.readInt1(), payload.readInt1()).atStartOfDay());
    }
    
    private Timestamp getTimestampForDatetime(final XuguPacketPayload payload) {
    ...
    private void writeDate(final XuguPacketPayload payload, final int year, final int month, final int dayOfMonth) {
