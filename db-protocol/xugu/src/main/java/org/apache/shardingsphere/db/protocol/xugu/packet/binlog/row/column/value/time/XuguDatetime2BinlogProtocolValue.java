...
package org.apache.shardingsphere.db.protocol.xugu.packet.binlog.row.column.value.time;
 ...
 */
public final class XuguDatetime2BinlogProtocolValue implements XuguBinlogProtocolValue {
    ...
    public Serializable read(final XuguBinlogColumnDef columnDef, final XuguPacketPayload payload) {
        ...
        return 0L == datetime ? XuguTimeValueUtils.DATETIME_OF_ZERO : readDatetime(columnDef, datetime, payload);
    }
    
    private long readDatetimeV2FromPayload(final XuguPacketPayload payload) {
        long result = 0L;
        for (int i = 4; i >= 0; i--) {
    ...
    private Serializable readDatetime(final XuguBinlogColumnDef columnDef, final long datetime, final XuguPacketPayload payload) {
        long datetimeWithoutSign = datetime & (0x8000000000L - 1L);
        if (0 == datetimeWithoutSign) {
            return XuguTimeValueUtils.DATETIME_OF_ZERO;
        }
        ...
        XuguFractionalSeconds fractionalSeconds = new XuguFractionalSeconds(columnDef.getColumnMeta(), payload);
