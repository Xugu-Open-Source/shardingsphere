...
public enum XuguCharacterSet {
    ...
    private static final Map<Integer, XuguCharacterSet> CHARACTER_SET_MAP = Collections.unmodifiableMap(Arrays.stream(values()).collect(Collectors.toMap(each -> each.id, Function.identity())));
    
    private final int id;
    
    private final Charset charset;
    
    XuguCharacterSet(final int id, final Supplier<Charset> charsetSupplier) {
        this.id = id;
        Charset result = null;
        try {
     ...
     */
    public static XuguCharacterSet findById(final int id) {
        XuguCharacterSet result = CHARACTER_SET_MAP.get(id);
