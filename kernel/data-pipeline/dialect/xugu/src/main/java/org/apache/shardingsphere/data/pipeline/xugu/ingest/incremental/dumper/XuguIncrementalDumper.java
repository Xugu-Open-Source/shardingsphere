...
package org.apache.shardingsphere.data.pipeline.xugu.ingest.incremental.dumper;
...
import org.apache.shardingsphere.data.pipeline.xugu.ingest.incremental.binlog.data.XuguBinlogDataHandler;
import org.apache.shardingsphere.data.pipeline.xugu.ingest.incremental.binlog.event.XuguBaseBinlogEvent;
import org.apache.shardingsphere.data.pipeline.xugu.ingest.incremental.binlog.event.rows.XuguBaseRowsBinlogEvent;
...
import org.apache.shardingsphere.data.pipeline.xugu.ingest.incremental.binlog.position.XuguBinlogPosition;
import org.apache.shardingsphere.data.pipeline.xugu.ingest.incremental.client.ConnectInfo;
import org.apache.shardingsphere.data.pipeline.xugu.ingest.incremental.client.XuguBinlogClient;
...
@Slf4j
public final class XuguIncrementalDumper extends AbstractPipelineLifecycleRunnable implements IncrementalDumper {
    
    private final IncrementalDumperContext dumperContext;
    
    private final XuguBinlogPosition binlogPosition;
    
    private final PipelineTableMetaDataLoader metaDataLoader;
    
    private final PipelineChannel channel;
    
    private final XuguBinlogClient client;
    
    private final String catalog;
    
    public XuguIncrementalDumper(final IncrementalDumperContext dumperContext, final IngestPosition binlogPosition, final PipelineChannel channel, final PipelineTableMetaDataLoader metaDataLoader) {
        this.dumperContext = dumperContext;
        this.binlogPosition = (XuguBinlogPosition) binlogPosition;
        ...
        client = new XuguBinlogClient(connectInfo, dumperContext.isDecodeWithTX());
    ...
    private void handleEvents(final List<XuguBaseBinlogEvent> events) {
        List<Record> dataRecords = new LinkedList<>();
        for (XuguBaseBinlogEvent each : events) {
    ...
    private List<? extends Record> handleEvent(final XuguBaseBinlogEvent event) {
        if (!(event instanceof XuguBaseRowsBinlogEvent)) {
            return Collections.singletonList(createPlaceholderRecord(event));
        }
        XuguBaseRowsBinlogEvent rowsEvent = (XuguBaseRowsBinlogEvent) event;
        ...
        if (event instanceof XuguWriteRowsBinlogEvent) {
            return handleWriteRowsEvent((XuguWriteRowsBinlogEvent) event, tableMetaData);
        }
        if (event instanceof XuguUpdateRowsBinlogEvent) {
            return handleUpdateRowsEvent((XuguUpdateRowsBinlogEvent) event, tableMetaData);
        }
        if (event instanceof XuguDeleteRowsBinlogEvent) {
            return handleDeleteRowsEvent((XuguDeleteRowsBinlogEvent) event, tableMetaData);
        }
        return Collections.emptyList();
    }
    
    private PlaceholderRecord createPlaceholderRecord(final XuguBaseBinlogEvent event) {
        PlaceholderRecord result = new PlaceholderRecord(new XuguBinlogPosition(event.getFileName(), event.getPosition()));
    ...
    private List<DataRecord> handleWriteRowsEvent(final XuguWriteRowsBinlogEvent event, final PipelineTableMetaData tableMetaData) {
        ...
        for (Serializable[] each : event.getAfterRows()) {
            ...
            for (int i = 0; i < each.length; i++) {
                ...
                dataRecord.addColumn(new NormalColumn(columnMetaData.getName(), XuguBinlogDataHandler.handle(columnMetaData, each[i]), true, columnMetaData.isUniqueKey()));
            }
    ...
    private List<DataRecord> handleUpdateRowsEvent(final XuguUpdateRowsBinlogEvent event, final PipelineTableMetaData tableMetaData) {
        List<DataRecord> result = new LinkedList<>();
        for (int i = 0; i < event.getBeforeRows().size(); i++) {
            ...
            for (int j = 0; j < beforeValues.length; j++) {
                ...
                Serializable oldValue = XuguBinlogDataHandler.handle(columnMetaData, beforeValues[j]);
                Serializable newValue = XuguBinlogDataHandler.handle(columnMetaData, afterValues[j]);
    ...
    private List<DataRecord> handleDeleteRowsEvent(final XuguDeleteRowsBinlogEvent event, final PipelineTableMetaData tableMetaData) {
        List<DataRecord> result = new LinkedList<>();
        for (Serializable[] each : event.getBeforeRows()) {
            ...
            for (int i = 0, length = each.length; i < length; i++) {
                PipelineColumnMetaData columnMetaData = tableMetaData.getColumnMetaData(i + 1);
                dataRecord.addColumn(new NormalColumn(columnMetaData.getName(), XuguBinlogDataHandler.handle(columnMetaData, each[i]), null, true, columnMetaData.isUniqueKey()));
            }
            result.add(dataRecord);
        }
        return result;
    ...
    private DataRecord createDataRecord(final PipelineSQLOperationType type, final XuguBaseRowsBinlogEvent rowsEvent, final int columnCount) {
        String tableName = dumperContext.getCommonContext().getTableNameMapper().getLogicTableName(rowsEvent.getTableName()).toString();
        IngestPosition binlogPosition = new XuguBinlogPosition(rowsEvent.getFileName(), rowsEvent.getPosition());
