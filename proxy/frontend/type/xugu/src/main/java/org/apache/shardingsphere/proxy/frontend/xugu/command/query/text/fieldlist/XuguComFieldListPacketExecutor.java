...
package org.apache.shardingsphere.proxy.frontend.xugu.command.query.text.fieldlist;
...
public final class XuguComFieldListPacketExecutor implements CommandExecutor {
    
    private static final String SQL = "SHOW COLUMNS FROM %s FROM %s";
    
    private final XuguComFieldListPacket packet;
    ...
    private Collection<DatabasePacket> createColumnDefinition41Packets(final String databaseName) throws SQLException {
        ...
        while (databaseConnector.next()) {
            ...
            result.add(new XuguColumnDefinition41Packet(
                    characterSet, databaseName, packet.getTable(), packet.getTable(), columnName, columnName, 100, XuguBinaryColumnType.VARCHAR, 0, true));
        }
        result.add(new XuguEofPacket(ServerStatusFlagCalculator.calculateFor(connectionSession, true)));
        return result;
    }
