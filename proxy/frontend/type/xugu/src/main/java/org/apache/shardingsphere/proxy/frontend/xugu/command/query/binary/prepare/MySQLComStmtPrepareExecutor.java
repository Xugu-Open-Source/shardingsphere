...
public final class MySQLComStmtPrepareExecutor implements CommandExecutor {
    ...
    private final XuguComStmtPreparePacket packet;
    ...
    private void failedIfContainsMultiStatements() {
        ...
        if (connectionSession.getAttributeMap().hasAttr(XuguConstants.OPTION_MULTI_STATEMENTS_ATTRIBUTE_KEY)
    ...
    private Collection<DatabasePacket> createPackets(final SQLStatementContext sqlStatementContext, final int statementId, final MySQLServerPreparedStatement serverPreparedStatement) {
        ...
        result.add(new XuguComStmtPrepareOKPacket(statementId, projections.size(), parameterCount, 0));
    ...
    private Collection<XuguPacket> createParameterColumnDefinition41Packets(final SQLStatementContext sqlStatementContext, final int characterSet,
        ...
        for (int index = 0; index < parameterMarkerSegments.size(); index++) {
            ...
            if (null != column) {
                ...
                result.add(createMySQLColumnDefinition41Packet(characterSet, columnDefinitionFlag, XuguBinaryColumnType.valueOfJDBCType(column.getDataType())));
                paramColumnDefinitionFlags.add(columnDefinitionFlag);
            } else {
                result.add(createMySQLColumnDefinition41Packet(characterSet, 0, XuguBinaryColumnType.VAR_STRING));
    ...
    private Collection<XuguPacket> createProjectionColumnDefinition41Packets(final SelectStatementContext selectStatementContext, final int characterSet) {
        ...
        Collection<XuguPacket> result = new ArrayList<>(projections.size());
        for (Projection each : projections) {
            ...
            if (each instanceof ColumnProjection) {
                result.add(Optional.ofNullable(schema.getTable(((ColumnProjection) each).getOriginalTable().getValue()))
                        ...
                        .map(column -> createMySQLColumnDefinition41Packet(characterSet, calculateColumnDefinitionFlag(column), XuguBinaryColumnType.valueOfJDBCType(column.getDataType())))
                        .orElseGet(() -> createMySQLColumnDefinition41Packet(characterSet, 0, XuguBinaryColumnType.VAR_STRING)));
            } else {
                result.add(createMySQLColumnDefinition41Packet(characterSet, 0, XuguBinaryColumnType.VAR_STRING));
            }
        }
    ...
    private int calculateColumnDefinitionFlag(final ShardingSphereColumn column) {
        int result = 0;
        result |= column.isPrimaryKey() ? XuguColumnDefinitionFlag.PRIMARY_KEY.getValue() : 0;
        result |= column.isUnsigned() ? XuguColumnDefinitionFlag.UNSIGNED.getValue() : 0;
        return result;
    }
    
    private XuguColumnDefinition41Packet createMySQLColumnDefinition41Packet(final int characterSet, final int columnDefinitionFlag, final XuguBinaryColumnType columnType) {
        return new XuguColumnDefinition41Packet(characterSet, columnDefinitionFlag, "", "", "", "", "", 0, columnType, 0, false);
    }
}
