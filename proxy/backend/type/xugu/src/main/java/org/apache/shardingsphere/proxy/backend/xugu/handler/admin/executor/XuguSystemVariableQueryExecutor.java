...
package org.apache.shardingsphere.proxy.backend.xugu.handler.admin.executor;
...
public final class XuguSystemVariableQueryExecutor implements DatabaseAdminQueryExecutor {
    
    private final List<ExpressionProjectionSegment> projections;
    
    private final List<XuguSystemVariable> variables;
    ...
    /**
     * Try to get {@link XuguSystemVariableQueryExecutor} for select statement.
     *
     * @param selectStatement select statement
     * @return {@link XuguSystemVariableQueryExecutor}
     */
    public static Optional<DatabaseAdminExecutor> tryGetSystemVariableQueryExecutor(final SelectStatement selectStatement) {
        ...
        List<XuguSystemVariable> variables = new ArrayList<>(projections.size());
        for (ProjectionSegment each : projections) {
            ...
            Optional<XuguSystemVariable> systemVariable = XuguSystemVariable.findSystemVariable(variable.getVariable());
            if (!systemVariable.isPresent()) {
                return Optional.empty();
            }
            variables.add(systemVariable.get());
        }
        return Optional.of(new XuguSystemVariableQueryExecutor(expressionProjectionSegments, variables));
    }
}
