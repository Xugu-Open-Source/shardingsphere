...
public final class XuguSetVariableAdminExecutor implements DatabaseAdminExecutor {
    ...
    private void validateSessionVariables(final Collection<String> sessionVariables) {
        for (String each : sessionVariables) {
            XuguSystemVariable systemVariable = XuguSystemVariable.findSystemVariable(each).orElseThrow(() -> new UnknownSystemVariableException(each));
