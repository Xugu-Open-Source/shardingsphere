...
import org.apache.shardingsphere.proxy.backend.xugu.handler.admin.executor.KillProcessExecutor;
import org.apache.shardingsphere.proxy.backend.xugu.handler.admin.executor.XuguSetVariableAdminExecutor;
import org.apache.shardingsphere.proxy.backend.xugu.handler.admin.executor.XuguSystemVariableQueryExecutor;
import org.apache.shardingsphere.proxy.backend.xugu.handler.admin.executor.NoResourceShowExecutor;
import org.apache.shardingsphere.proxy.backend.xugu.handler.admin.executor.ShowConnectionIdExecutor;
import org.apache.shardingsphere.proxy.backend.xugu.handler.admin.executor.ShowCreateDatabaseExecutor;
import org.apache.shardingsphere.proxy.backend.xugu.handler.admin.executor.ShowCurrentDatabaseExecutor;
import org.apache.shardingsphere.proxy.backend.xugu.handler.admin.executor.ShowCurrentUserExecutor;
import org.apache.shardingsphere.proxy.backend.xugu.handler.admin.executor.ShowDatabasesExecutor;
import org.apache.shardingsphere.proxy.backend.xugu.handler.admin.executor.ShowFunctionStatusExecutor;
import org.apache.shardingsphere.proxy.backend.xugu.handler.admin.executor.ShowProcedureStatusExecutor;
import org.apache.shardingsphere.proxy.backend.xugu.handler.admin.executor.ShowProcessListExecutor;
import org.apache.shardingsphere.proxy.backend.xugu.handler.admin.executor.ShowTablesExecutor;
import org.apache.shardingsphere.proxy.backend.xugu.handler.admin.executor.ShowVersionExecutor;
import org.apache.shardingsphere.proxy.backend.xugu.handler.admin.executor.UnicastResourceShowExecutor;
import org.apache.shardingsphere.proxy.backend.xugu.handler.admin.executor.UseDatabaseExecutor;
...
public final class XuguAdminExecutorCreator implements DatabaseAdminExecutorCreator {
    ...
    private Optional<DatabaseAdminExecutor> findAdminExecutorForSelectWithoutFrom(final String sql, final String databaseName, final SelectStatement selectStatement) {
        Optional<DatabaseAdminExecutor> result = XuguSystemVariableQueryExecutor.tryGetSystemVariableQueryExecutor(selectStatement);
    ...
    private boolean isQuerySysdbaSchema(final String databaseName) {
        // TODO remove DefaultDatabaseMetaDataExecutor when sql federation can support all system table query
        return SYSDBA.equalsIgnoreCase(databaseName) && !ProxyContext.getInstance().getContextManager().getDatabase(databaseName).isComplete();
    }
